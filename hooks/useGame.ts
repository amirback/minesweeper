...
function emptyStats(): LocalStats {
  return {
    totalGames: 0, wins: 0, totalClicks: 0, mineHits: 0, totalTime: 0,
    byDifficulty: {
...
export function useGame(initialDifficulty: Difficulty = 'easy', options: UseGameOptions = {}) {
  ...
  const handleGameEnd = useCallback(async (won: boolean, finalTimer: number) => {
    ...
    if (mode === 'daily' && won) {
      ...
      if (userId) {
        await saveGameResult({ user_id: userId, difficulty: dailyDifficulty, time_seconds: finalTimer, won: true, is_daily: true, daily_date: getDailyDateString(), elo_delta: gain });
      }
    } else if (userId) {
      await saveGameResult({ user_id: userId, difficulty, time_seconds: finalTimer, won, is_daily: false, elo_delta: gain });
    }
  ...
  const handleCellClick = useCallback(
    (row: number, col: number, clientX?: number, clientY?: number) => {
      ...
      if (flagMode) {
        ...
        if (!board[row][col].isFlagged && flagsUsed >= diffConfig.maxFlags) return;
        ...
        if (placing) {
          ...
          setFlagEvent({ id: flagEventIdRef.current, x: clientX ?? window.innerWidth / 2, y: clientY ?? window.innerHeight / 2, isMine: board[row][col].isMine, gameActive: status === 'playing' });
        } else {
          sounds.click();
        }
        setBoard(newBoard);
      ...
      const { rows, cols, mines } = diffConfig;
      ...
      if (currentBoard[row][col].isRevealed) {
        ...
        if (hitMine) {
          sounds.explosion(); breakCombo();
          const stats = loadStats(); stats.totalClicks++; stats.mineHits++; saveStats(stats);
          setBoard(revealAllMines(currentBoard)); setStatus('lost'); handleGameEnd(false, timer);
          return;
        }
        let afterChord = currentBoard;
        for (const [nr, nc] of unrevealed) afterChord = revealCell(afterChord, nr, nc, rows, cols);
        sounds.cascade(); bumpCombo();
        const stats = loadStats(); stats.totalClicks++; saveStats(stats);
        if (checkWin(afterChord)) { setBoard(afterChord); setStatus('won'); sounds.victory(); breakCombo(); handleGameEnd(true, timer); return; }
        setBoard(afterChord);
        return;
      }

      if (currentBoard[row][col].isFlagged) return;
      const stats = loadStats(); stats.totalClicks++;
      if (currentBoard[row][col].isMine) {
        stats.mineHits++; saveStats(stats); sounds.explosion(); breakCombo();
        setBoard(revealAllMines(currentBoard)); setStatus('lost'); handleGameEnd(false, timer);
        return;
      }
      saveStats(stats);
      const newBoard = revealCell(currentBoard, row, col, rows, cols);
      sounds.click(); bumpCombo();
      if (checkWin(newBoard)) { setBoard(newBoard); setStatus('won'); sounds.victory(); breakCombo(); handleGameEnd(true, timer); return; }
      setBoard(newBoard);
    },
  ...
  const handleCellRightClick = useCallback(
    (row: number, col: number, clientX = 0, clientY = 0) => {
      if (status === 'won' || status === 'lost') return;
      if (board[row][col].isRevealed) return;
      if (!board[row][col].isFlagged && flagsUsed >= DIFFICULTY_CONFIG[difficulty].maxFlags) return;
      const newBoard = toggleFlag(board, row, col);
      const placing = newBoard[row][col].isFlagged;
      if (placing) {
        sounds.flag();
        setFlagsUsed(u => u + 1);
        flagEventIdRef.current++;
        setFlagEvent({ id: flagEventIdRef.current, x: clientX, y: clientY, isMine: board[row][col].isMine, gameActive: status === 'playing' });
      } else {
        sounds.click();
      }
      setBoard(newBoard);
    ...
    [board, status, difficulty, flagsUsed]
  );
  ...
  const activeDiff = mode === 'daily' ? dailyDifficulty : difficulty;

  return {
    board, status, difficulty, flagsPlaced, flagsUsed, timer,
    showProbability, probabilities, flagMode,
    minesTotal: DIFFICULTY_CONFIG[activeDiff].mines,
    maxFlags:   DIFFICULTY_CONFIG[activeDiff].maxFlags,
    dailyCompleted, combo, eloGain, flagEvent,
